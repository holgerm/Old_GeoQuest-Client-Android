...
public class LocationSource {
	...
	URL url = null;
	try {
	    url = new URL(LOCATION_MOCK_SERVER_URL
		    + "?get=1&id="
		    + getDeviceID(context));
	} catch (MalformedURLException e) {
	    Log.e(TAG,
		  "makeURLForGetPosition() constructed this url: "
			  + LOCATION_MOCK_SERVER_URL
			  + "?get=1&id="
			  + getDeviceID(context));
	}
	return url;
    ...
    public static boolean setDeviceName(Context context,
					String deviceName) {
	String deviceID = getDeviceID(context);
	StringBuilder responseString = new StringBuilder();
	try {
	    URL url = new URL(LOCATION_MOCK_SERVER_URL
		    + "?set=1&id="
		    + deviceID
		    + "&name="
		    + deviceName);
	...
	return (response.startsWith("ok "
		+ deviceID) && response.endsWith(deviceName));
    }
    ...
    public LocationSource(Context context,
			  SmartLocationListener locationListener,
			  Handler handler,
	...
	try {
		...
		locationManager
			.addTestProvider(MockLocationReceiver.MOCK_PROVIDER,
					 true,
					 false,
					 false,
					 false,
	...
	case REAL_MODE:
	    ...
	    locationManager
		    .setTestProviderEnabled(MockLocationReceiver.MOCK_PROVIDER,
					    false);
	    locationManager
	...
	default:
	    throw new IllegalArgumentException(
		    "No such location source mode defined: "
			    + newMode);
	}
	return newMode;
    }
	...
	public void run() {
		...
		if ((newLocation != null)
			&& (firstRunSinceModeActivated
				|| oldLatitude != newLocation.getLatitude() || oldLongitude != newLocation
				.getLongitude())) {
