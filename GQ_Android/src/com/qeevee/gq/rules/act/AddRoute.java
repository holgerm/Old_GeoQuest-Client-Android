	...
	public void execute() {
		...
		if (fromHs == null) {
			if (params.get("from").equals("currentPos")) {
				...
				Location location = LocationUtilities
						.getCurrentLocation(GeoQuestApp.getInstance()
		...
		if (isCommingFromPlayerLocation)
			route.setCommingFromPlayerLocation(true);
		if (isEndingAtPlayerLocation)
			route.setEndingAtPlayerLocation(true);
